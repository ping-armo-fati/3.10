...
ASSETS_DIR = "../../assets"         # the dir of the pngs
MARKDOWN_PROFIX = "../../assets/"
    ...
    with open(md_path, 'r', encoding='utf-8') as f_md:
        with open(tmp_path, 'w', encoding='utf-8') as f_tmp:
            ...
            for i, name in enumerate(image_names):
                name_with_prefix = MARKDOWN_PROFIX + name
                body_md = re.sub("media\/image" + str(i + 1) + "\.[a-zA-Z]+" , name_with_prefix, body_md)
            f_tmp.write(body_md)
...
def handle_page(onenote, elem, path, i):
    ...
    try:
        ...
        os.system('pandoc.exe -i "%s" -o "%s" -t markdown-simple_tables-multiline_tables-grid_tables --wrap=none' % (
        path_docx, path_md))
...
if __name__ == "__main__":
    try:
        ...
        heer = onenote.GetHierarchy("", win32.constants.hsNotebooks, "")

        root = ElementTree.fromstring(heer)
        for child in root:
